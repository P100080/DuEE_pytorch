    ...
    for epoch in range(args.num_epoch):
        for step, batch in enumerate(train_loader):
            ...
            out = model(input_ids=batch['all_input_ids'], attention_mask=batch['all_attention_mask'],
                           token_type_ids=batch['all_token_type_ids'],label_ids=batch['all_labels'])
            #logits=out["logits"]
...
def do_predict():
    ...
    '''model = DuEE_model(
        "bert-base-chinese", num_classes=len(label_map))'''
    model=Bert_CRF("bert-base-chinese",num_classes=len(label_map))
    ...
    for batch in tqdm(test_loader, total=len(test_loader)):
        ...
        #input对应的最大可能的标签
        probs_ids=out#torch.argmax(logits, axis=-1)
