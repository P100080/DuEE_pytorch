...
pred_data=${data_dir}/duee_test2.json   # 换其他数据，需要修改它
...
run_sequence_labeling_model(){
    ...
    sh run_sequence_labeling.sh ${data_dir}/${model} ${conf_dir}/${model}_tag.dict ${ckpt_dir}/${model} ${pred_data} ${learning_rate} ${is_train} ${max_seq_len} ${batch_size} ${epoch} ${pred_save_path} ${hidden_dim} ${rnn_layers} ${lstm_dropout_ratio} ${dropout}
}
...
if [ ${process_name} == data_prepare ]; then
    ...
    python duee_1_postprocess.py --trigger_file ${ckpt_dir}/trigger/test_pred.json --role_file ${ckpt_dir}/role/test_pred.json --schema_file ${conf_dir}/duee_event_schema.json --save_path ${submit_data_path}
